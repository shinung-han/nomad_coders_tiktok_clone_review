...
class S {
  ...
  String signUpTitle(String nameOfTheApp, DateTime when) {
    final DateFormat whenDateFormat =
        DateFormat('MMMMEEEEd 🕺🏻 QQQQ', Intl.getCurrentLocale());
  ...
  /// `Log in {gender, select, male{sir} femail{madam} other{human}}`
  String logIn(Object gender) {
    return Intl.message(
      'Log in ${Intl.select(gender, {
            'male': 'sir',
            'femail': 'madam',
            'other': 'human'
          })}',
